...
<div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
    <div class="flex items-center justify-between gap-4">
        <div class="sm:flex-auto">
            <flux:heading level="3">{{ __('Edit user') }}</flux:heading>
            <flux:text class="hidden sm:inline">{{ __('Edit user data') }}</flux:text>
        </div>
    </div>

    <form wire:submit="updateUser" class="my-6 w-full max-w-xl space-y-6">
        <!-- Name -->
        <flux:input
                wire:model="name"
                :label="__('Name')"
